...
To detect the traffic lights,  the system (and in particular the Perception module) should have a model that can detect correctly traffic lights and its state (red, yellow or green). Furthermore, apart from a high degree accuracy (as a detection error could cause a car accident), the traffic light classifier might need to run fast, i.e. be efficient.

## Models and architectures

We considered different models and arquitectures to base our traffic light detection:
...
We used different models with different steps, and we evaluated the performance based on the four categories:
