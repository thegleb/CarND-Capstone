...
![](./img/sim_labeled_original.png "Sample labeled training image")

Example of a synthetic image for evaluation:

![](./img/sim_synthetic_1.jpg "Sample labeled synthetic image")
...
We tried several datasets for training the site models. One was a labeled set of images from the `traffic_light_training.bag` data set. We also tried training with synthetic images created by amending one of these images with another instance of a traffic light:

![](./img/site_synthetic_1.png "Sample site synthetic image")

We also trained models using images created in a way similar to the synthetic simulator images - using a clean background with multiple traffic light images superimposed. Additionally, the image was contrast-adjusted using OpenCV and the CLAHE algorithm, and the gamma value of each image was varied between values of 0.4 to 0.6, meaning the overall brightness was brought down to emphasize the details in the traffic lights.

![](./img/site_synthetic_2.jpg "Another sample site synthetic image")
...
- ssd_mobilenet_v1_coco_20000ssd_mobilenet_v1_coco_20000_gamma
...
![](./img/site_models_evaluation.png "Model performance using site data")

We also observed that by adjusting the gamma in the different images influenced how well the models performed. That is why, we used another test set to be able to adjust the gamma parameter for the ssd_mobilenet_v1_coco_20000_gamma. The results are below.

![](./img/ssd_mobilenet_v1_coco_20000_gamma_analysis.png "Gamma analysis for the ssd mobilenet v1 coco 20000")

We decided to adjust the gammma parameter to 0.4 as this showed better results.

Finally, we evaluated the final model with an evaluation data set (real data - non-generated by a script).

![](./img/ssd_mobilenet_v1_coco_20000_evaluation.png "Evaluation of the ssd mobilenet v1 coco 20000 with evaluation data set")
